...
if [ "$NODE_TYPE" = "chunk_holder" ] || [ "$NODE_TYPE" = "infra" ]; then
    ...
    while IFS=' ' read -ra CHUNK; do
        #ignore comments in file, and iterate through the file contents that have been dumped in
        [[ "$CHUNK" =~ ^#.*$ ]] && continue
        #start up a server
        docker run \
            -d \
...
if [ "$NODE_TYPE" = "client" ]; then
    ...
    # generate a random ip for the client to using the second half of the subnet (128-255), hardcoded ip subnet
    # for right now
    _ip=$(printf "10.0.1.%d" "$((RANDOM % 128 + 128 ))")

    # run the client
    docker run \
        --name "${NAME_PREPEND}-client-${_number}" \
        --network="$DOCKER_NETWORK_NAME" \
        --ip "$_ip" \
        "$DOCKER_IMAGE" \
